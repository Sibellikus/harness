...
into a bounded implementation task, build it, prove it, review it, and ship it.

1. Shape: `shape` hardens raw requirements into an
   implementation-ready plan with file-level tasks and BDD-style acceptance
   criteria.
2. Build: `build` turns one READY slice from that plan into code without
   inventing missing decisions. If the plan is incomplete, it stops with a plan
   gap.
3. Prove: `prove` plans the verification pass after code changes, including
   the test surface and acceptance evidence to collect.
4. Review: `review`, `deep-review`, `frontend-ux-review`, and
   `idempotency-audit` cover review gates for code correctness, current diffs,
   rendered UI, and retry/idempotency risks.
5. Ship: `ship` runs the final repository-owned delivery gate before pushing or
   opening a PR.
...
- `codex/skills/shape` - planning and task-hardening workflow.
- `codex/skills/build` - bounded builder for READY shape plans.
- `codex/skills/prove` - post-implementation test planning workflow.
- `codex/skills/review` - current-diff review workflow.
- `codex/skills/ship` - repository-owned check/push/PR guardrail.
